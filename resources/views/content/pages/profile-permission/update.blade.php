...
@section('content')
  <div class="col-md">
    <h4 class="mb-4">
      <span class="text-muted fw-light">Perfil / Permissões /</span> Atualizar
    </h4>
    <div class="card mb-4">
      {{-- <h5 class="card-header">Novo chamado</h5> --}}
      <div class="card-body">
        <form method="POST" action="{{ route('sys-permission-update', $profilePermission->id) }}">
          @csrf
          ...
          <div class="row mb-4">
                ...
                <select id="sidebar" name="sidebar" class="form-select" disabled>
                  @foreach ($sidebars as $sidebar)
                    <option value="{{ $sidebar->id }}"
                      {{ str_contains($profilePermission->sidebar_id, $sidebar->id) ? 'selected' : '' }}>
                      {{ $sidebar->name }}</option>
                  @endforeach
                </select>
              </div>
                ...
                <select id="profile" name="profile" class="form-select">
                  @foreach ($profiles as $profile)
                    <option value="{{ $profile->id }}"
                      {{ str_contains($profilePermission->profile_id, $profile->id) ? 'selected' : '' }}>
                      {{ $profile->name }}
                    </option>
                  @endforeach
                </select>
            ...
            <div class="col-md-8">
              ...
              <textarea class="form-control" placeholder="Descreva o que precisa ser feito no chamado em questão." id="description"
                name="description" maxlength="250" rows="4"> {{ $profilePermission->description }} </textarea>
            </div>
          </div>
            ...
            <div class="d-flex justify-content-between">
              ...
              <div class="form-check ">
                <input class="form-check-input" type="checkbox" name="listCheck"
                  {{ $profilePermission->list == 1 ? 'checked' : '' }} />
              ...
              <div class="form-check ">
                <input class="form-check-input" type="checkbox" name="createCheck"
                  {{ $profilePermission->create == 1 ? 'checked' : '' }} />
              ...
              <div class="form-check ">
                <input class="form-check-input" type="checkbox" name="updateCheck"
                  {{ $profilePermission->update == 1 ? 'checked' : '' }} />
              ...
              <div class="form-check">
                <input class="form-check-input" type="checkbox" name="deleteCheck"
                  {{ $profilePermission->delete == 1 ? 'checked' : '' }} />
