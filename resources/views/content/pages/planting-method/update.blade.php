...
@section('content')
  <div class="col-md">
    ...
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('cultive-method-update', $plantingMethod->id) }}">
          @csrf
          ...
          <div class="row mb-4">
            <div class="col-md-4">
              ...
              <input type="text" class="form-control" id="code" name="code" maxlength="50"
                value="{{ $plantingMethod->code }}" />
            </div>

            <div class="col-md-4">
              <label for="name" class="form-label">Nome</label>
              <input type="text" class="form-control" id="name" name="name" maxlength="50"
                value="{{ $plantingMethod->name }}" />
            </div>

            <div class="col-md-4">
              <label for="situation" class="form-label">Situação</label>
              <select id="situation" name="situation" class="form-select">
                <option value="0" {{ str_contains($plantingMethod->situation, 0) ? 'selected' : '' }}>Inativo
                </option>
                <option value="1" {{ str_contains($plantingMethod->situation, 1) ? 'selected' : '' }}>Ativo</option>
              </select>
            </div>
          </div>
