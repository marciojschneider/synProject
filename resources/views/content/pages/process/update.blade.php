...
@section('content')
  <div class="col-md">
    <h4 class="mb-4">
      <span class="text-muted fw-light">Cultivo / Processo/Etapas /</span> Atualizar
    </h4>
    <div class="card mb-4">
      {{-- <h5 class="card-header">Novo chamado</h5> --}}
      <div class="card-body">
        <form method="POST" action="{{ route('cultive-process-update', $process->id) }}">
          @csrf
          ...
          <div class="row mb-4">
            ...
            <div class="col-md-3">
              ...
              <select id="type" name="type" class="form-select">
                <option value="1" {{ str_contains($process->type, 1) ? 'selected' : '' }}>Produtivo</option>
                <option value="2" {{ str_contains($process->type, 2) ? 'selected' : '' }}>Não Produtivo</option>
              </select>
            </div>
            ...
            <div class="col-md-3">
              ...
              <select id="situation" name="situation" class="form-select">
                <option value="0" {{ str_contains($process->situation, 0) ? 'selected' : '' }}>Inativo
                </option>
                <option value="1" {{ str_contains($process->situation, 1) ? 'selected' : '' }}>Ativo</option>
              </select>
            </div>
          </div>
