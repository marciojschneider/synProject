<div class="col-md">
  ...
  <div class="card mb-4">
    ...
    <div class="card-body">
      <form method="POST" action="{{ route('sys-sec-closure-update', $closure_module->id) }}">
        @csrf
        <div class="row mb-4">
          <div class="col-md-3">
            ...
            <select id="module" name="module" class="form-select" wire:model.live="module">
              @foreach ($modules as $module)
                <option value="{{ $module->id }}"
                  {{ str_contains($closure_module->module, $module->id) ? 'selected' : '' }}>
            ...
            <select id="screen" name="screen" class="form-select">
              @foreach ($screens as $screen)
                <option value="{{ $screen->id }}"
                  {{ str_contains($closure_module->sidebar_id, $screen->id) ? 'selected' : '' }}>
          ...
          <div class="col-md-3">
            ...
            <select id="situation" name="situation" class="form-select">
              ...
              <option value="1" {{ str_contains($closure_module->situation, 1) ? 'selected' : '' }}>Ativo
              </option>
            </select>
          </div>
        </div>
