    ...
    <div class="col-md-10" bis_skin_checked="1">
      <div
        ...
        <div id="DataTables_Table_0_filter" class="dataTables_filter" bis_skin_checked="1" style="margin-right: 5px;">
          <select wire:model.live.click="farm" id="farm" name="farm" class="form-select">
            <option value="" selected> FAZENDA </option>
            @foreach ($farms as $farm)
              <option value="{{ $farm->id }}">{{ mb_strtoupper($farm->name, 'UTF-8') }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div id="DataTables_Table_0_filter" class="dataTables_filter" bis_skin_checked="1" style="margin-right: 5px;">
          <select wire:model.live.click="locality" id="locality" name="locality" class="form-select">
            <option value="" selected> LOCALIDADE </option>
            @foreach ($localities as $locality)
              <option value="{{ $locality->id }}">{{ mb_strtoupper($locality->name, 'UTF-8') }}</option>
            @endforeach
          </select>
        </div>
